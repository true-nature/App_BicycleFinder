...
static void SerCmdAscii_Output(tsSerCmd_Context *pc, tsFILE *ps) {
	SerCmdAscii_Output_AdrCmd(ps, pc->au8data[0], pc->au8data[1], &(pc->au8data[2]), pc->u16len - 2);
}
 ...
 */
void SerCmdAscii_Output_AdrCmd(tsFILE *psSerStream, uint8 u8addr, uint8 u8cmd, uint8 *pDat, uint16 u16len) {
	int i;
	uint8 u8lrc = 0;
